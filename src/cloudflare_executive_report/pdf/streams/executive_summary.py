...
def append_executive_summary(
    ...
    *,
    ...
    available_streams = summary.get("available_streams", {})

    # ------------------------------------------------------------------
    # Platform KPIs (always available)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # Traffic KPIs (skip if HTTP stream missing)
    # ------------------------------------------------------------------
    if available_streams.get("http", False):
    ...
    # ------------------------------------------------------------------
    # DNS KPIs (skip if DNS stream missing)
    # ------------------------------------------------------------------
    if available_streams.get("dns", False):
        dns = kpis.get("dns") or {}
        dns_records = kpis.get("dns_records") or {}
        dr_un = bool(dns_records.get("unavailable"))
        apex_status = "-" if dr_un else dns_records.get("apex_protection_status", "-")
    ...
    # ------------------------------------------------------------------
    # Audit & Certificates (skip if not available)
    # ------------------------------------------------------------------
    if available_streams.get("audit", False) and available_streams.get("certificates", False):
        ...
        cert_exp30_v = (
            UNAVAILABLE if ce_un else str(certificates_k.get("expiring_in_30_days") or "0")
        )
    ...
    # ------------------------------------------------------------------
    # Email security (skip if email stream missing)
    # ------------------------------------------------------------------
    if available_streams.get("email", False):
        email_k = kpis["email"]
